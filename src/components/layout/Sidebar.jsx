...
import { LayoutGrid, MapPinned, Tags, Map, Images, CalendarDays, MessageSquareText,
  Star, Heart, Trash2, MessageCircle, Settings, LogOut, ChevronRight, ChevronLeft,
  Sun, Moon, Folder, Clock } from "lucide-react";
import LogoutAlert from './LogoutAlert'; // Import the logout alert component
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { name: "Delete Account", icon: Trash2, path: "/delete-account" },
  ];
  ...
  const renderNavItem = (item) => {
    ...
    // Special handling for logout button - Red styling
    if (item.name === "Log out") {
      return (
        <button
          key={item.name}
          onClick={() => setShowLogoutAlert(true)}
          className={`group relative flex items-center w-full px-3 py-2.5 my-0.5 rounded-xl cursor-pointer transition-colors duration-200 text-red-600 hover:bg-red-50`}
        >
          <div className="flex items-center w-full">
            <div className={`flex items-center justify-center shrink-0 ${!isExpanded ? 'mx-auto' : ''}`}>
              <Icon size={20} strokeWidth={2} className="text-red-600" />
            </div>
            {isExpanded && (
              <span className={`ml-3 text-sm font-medium whitespace-nowrap text-red-600`}>
                {item.name}
              </span>
            )}
          </div>
          {!isExpanded && (
            <div className="absolute left-full ml-4 px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
              {item.name}
