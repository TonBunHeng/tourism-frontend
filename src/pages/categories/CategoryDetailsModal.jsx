      ...
      <div className="bg-[var(--color-bg-dark-modal)] text-[var(--color-white)] rounded-3xl max-w-lg w-full shadow-2xl border border-[var(--color-border-dark)] overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--color-border-dark)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-[var(--color-white)] tracking-wide">Category Details</h3>
              <p className="text-xs text-[var(--color-text-secondary-dark)]">ID: #{category.id}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary-dark)]">Category Name</span>
            <p className="text-base font-semibold text-[var(--color-white)] mt-1">{category.name}</p>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[var(--color-surface-hover-dark)]/50 p-3.5 rounded-2xl border border-[var(--color-border-dark)]">
              <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary-dark)]">Total Places</span>
          ...
          <div className="bg-[var(--color-surface-hover-dark)]/50 p-3.5 rounded-2xl border border-[var(--color-border-dark)]">
            ...
            <p className="text-sm text-[var(--color-text-secondary-dark)] mt-1">{category.description}</p>
          </div>

          <div className="bg-[var(--color-surface-hover-dark)]/50 p-3.5 rounded-2xl border border-[var(--color-border-dark)]">
            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary-dark)]">Created Date</span>
            <p className="text-sm text-[var(--color-text-secondary-dark)] mt-1">{category.createdAt}</p>
          </div>
        </div>
